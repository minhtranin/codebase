...
const StyledGrid = styled.div `
  ...
  grid-template-areas: "img addr addr services" "img data contact services" "img action action action";
  grid-gap: 8px 8px;
  min-height: 180px;
  @media (max-width: ${props => props.theme.breakpoints.BREAKPOINT_XL}) {
    grid-template-columns: 160px  minmax(195px, 1fr) 2fr 3fr;
    grid-template-areas: "addr addr addr services" "img data contact services" "action action action action";
    grid-gap: 4px 8px;
  }
  @media (max-width: ${props => props.theme.breakpoints.BREAKPOINT_LG}) {
    grid-template-columns: 180px 1fr 2fr;
    grid-template-rows: ${ADDRESS_BAR_HEIGHT} auto auto;
    grid-template-areas: "addr addr addr" "data contact services" "action action action";
    grid-gap: 4px 8px;
    padding-left: 4px;
  }
  @media (max-width: ${props => props.theme.breakpoints.BREAKPOINT_MD}) {
    ...
    grid-template-areas: "addr addr" "img data" "contact contact" "services services" "action action";
  }
  @media (max-width: ${props => props.theme.breakpoints.BREAKPOINT_SM}) {
    ...
    grid-template-areas: "addr" "img" "data" "contact" "services" "action";
  }
`
